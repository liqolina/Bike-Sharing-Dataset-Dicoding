...
def create_temp_df(df):
    temp_df =  df.groupby(by='temp')[['count': 'sum']].sum().reset_index() 
    return temp_df

# Mendefinisikan Suhu yang dirasakan Tubuh
def create_atemp_df(df):
    atemp_df =  df.groupby(by='atemp')[['count': 'sum']].sum().reset_index() 
    return atemp_df
...
def create_season_df(df):
    season_df = df.groupby(by='season')[['count': 'sum']].sum().reset_index() 
    return season_df
